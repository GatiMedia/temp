...
MATERIAL_LAYERS = ['diffuse', 'specular_direct', 'specular_indirect', 'specular',
                    'diffuse_direct', 'diffuse_indirect', 'coat', 'emission', 'emission_indirect', 'transmission',
                    'sheen', 'sss', 'volume', 'refraction']
...
def get_color_layers(channel_layers):
    ...
    for c in channel_layers:
        if c.startswith(COLOR_LAYER_PREFIX):
            ...
            continue
    if "emission" in channel_layers:
        color_group.append("LGT_emission")
...
def get_material_layers(channel_layers):
    ...
    material_group = []
    for c in channel_layers:
        if c in MATERIAL_LAYERS:
            material_group.append(str("M_"+c))
        else:
            continue
    # Removing passes that has "mask" in the name
...
def create_unpremult_group(start_point, layers):
    ...
    with group_unprem_color:
        ...
        for index, c_layers in enumerate(layers):
            if "M_" in c_layers:
                c_layers = c_layers.replace("M_","")
            if "LGT_emission" == c_layers:
                c_layers = "emission"
...
def loop_part(choosen_group, dotMain, choosen_type, texture_group):
    ...
    for index, n in enumerate(choosen_group):
        ...
        # creating shuffle
        n = n.replace("M_", "")
        n = n.replace("LGT_emission", "emission")
